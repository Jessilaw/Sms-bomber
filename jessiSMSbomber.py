...
def watermark():
    print("SMS Tool by coder JESSİ . İletişim: t.me/Datasatis ")
...
def get_amount():
    ...
    while True:
        try:
            amount = int(input(f"""Kaç SMS gönderilsin? Sınırsız gönderim için "00" basın.\n[?] : """))
            if amount >= 0:
                return amount
            else:
                clear()
...
def get_worker_amount():
    ...
    while True:
        try:
            worker_amount = int(input(f" kaç saniye aralıklarla göndermek istersiniz. Tavsiye edilen 5-100 arasıdır.\n[?] : "))
            if worker_amount >= 1:
